...
work on delay-tolerant email~\cite{pan2025}, which built and validated an ION and
...
running on an Azure virtual machine on port 5353. The reasons for using a private
resolver are given in the previous chapter.
...
Runs 2 and 3 are executed in the same pass, one after the other, so that the two
delayed conditions are separated by as little time as possible. External blocklist
state changes over hours, and comparing two runs made far apart measures that change
as much as it measures the effect of the configuration. The reasoning behind this is
developed with the results in Chapter~5.
