...
The DTN transport layer of the testbed follows the approach established in earlier
work on delay-tolerant email, which built and validated an ION and
BPMail pipeline for carrying mail over the Bundle Protocol. Same set of software has been used, but instead of being containerized, the software stack has been created on virtual machines due to the reasons mentioned in the previous chapter.

All the other components of the testbed architecture are new, including delay
introduction on the relay, external resolver, installation and setup of all the
anti-spam systems, the measurement methodology and the experimentation setup.

\section{Environment and Dependencies}

Five VMs are used in the testbed in an Apple Silicon MacBook Air in UTM. All five use
Ubuntu 22.04 LTS (ARM64). There are three servers called \textit{mail1}, \textit{space}
and \textit{mail2}; there are two clients – one for each network. The mail, DTN and
anti-spam applications are installed on the servers. Clients use Thunderbird and are only
used to verify that the mail transfer works correctly end-to-end and do not participate
in the measurement process.

There are three libraries which are more recent than those in the Ubuntu 22.04
repositories and have to be built from source on each server before ION and BPMail are
compiled: mbedtls 2.28.8, gmime 3.2.7 and c-ares 1.34.4. Each library is built in order
and then \texttt{ldconfig} is run to make sure that the libraries are found first.
...
disables non-standard platform macros such as \texttt{linux}, so the guard never
passed and the definition was never picked up, even when \texttt{-Dlinux} was passed
...
This path is what the Thunderbird clients use, and it is how the testbed was
verified as a working mail system. It is not the path used for measurement. The
experiments call \texttt{bpmailsend} and \texttt{bpmailrecv} directly, for the
reasons given in Section~\ref{sec:harness-agents}.

\section{Delay Simulation}

The link delay is applied on space, using \texttt{tc} with the \texttt{netem}
queueing discipline, attached to the Moon-facing interface. It therefore acts on all
traffic crossing the boundary between the two networks.

\texttt{tc} can only shape traffic leaving an interface. The outbound direction is
straightforward: a \texttt{netem} qdisc is attached to the root of the Moon-facing
interface. The inbound direction has no equivalent, because there is no egress queue
to attach a qdisc to. The usual way around this is an Intermediate Functional Block
(ifb) device, a virtual interface whose own egress path can be shaped normally.
Traffic arriving on the Moon-facing interface is matched by a filter and redirected
to the ifb device with a \texttt{mirred} action, and a second \texttt{netem} qdisc on
the ifb device applies the same delay. The full sequence is:
...
rspamd runs as a daemon and is scanned through its client:

\begin{lstlisting}
rspamc --ip 1.2.3.4 --mime < "$FILE"
\end{lstlisting}

The \texttt{--ip} option supplies a client address so that the message is treated as
having arrived from outside. Without it, mail presented locally is treated as
internal and the network checks that this study is concerned with are skipped.
...
The corpus is drawn from Richard Clayton's public spam archive, a collection of real
messages captured from live mail traffic. From it, 681 messages were used for every
run. The same 681 messages, in the same sorted order, were scanned in all three runs
of every tool, so that per-message comparisons across runs refer to identical inputs.
The corpus is used to characterise tool behaviour under delay rather than to measure
absolute detection accuracy, so what matters here is that the message set is fixed and
realistic, not that it is balanced between spam and legitimate mail.
...
Every parameter it uses is held in one configuration file, so the procedure is
identical between tools and between passes, and the file itself records the exact
settings a set of results was produced under.
...
\begin{itemize}
  ...
  \item \textbf{Tool.} The tool's version command must return something, which
  confirms it is installed; the version string is recorded with the results.
\end{itemize}
...
The tool profile for the run is then applied, and the delay if the run calls for it.
The round-trip time is then measured by pinging mail1 from mail2, and checked against
a window: 2400 to 2900\,ms for a delayed run, and under 150\,ms for the baseline. A
measurement outside the window aborts the run. This is the most important gate in the
harness. A run whose delay was not actually in place, or was applied twice, would
produce plausible-looking numbers that mean nothing, and the failure would not be
obvious afterwards.
...
minutes, then writes a \texttt{STOP} file that the receiver checks between bundles so
that it exits cleanly rather than being killed mid-scan.
...
resolver stopped working properly during the run. This is checked automatically so
that a compromised run is flagged at the time rather than discovered later.
...
Each run finally writes a manifest recording the tool and version, the profile, the
measured round-trip time and the gate it was checked against, the timings, the
message counts, the resolver health figures and the resulting status. The manifest is
what makes a set of results interpretable months later.
...
The testbed is a working DTN mail system with a controllable link delay, a resolver
that reaches real blocklists, and three anti-spam tools each configurable into a
default and a reduced-timeout condition. The experiments are driven by a
configuration-file-based harness that verifies the testbed before each run, refuses
to proceed if the delay is not what it should be, collects every artifact, and flags
runs whose resolver state indicates the results cannot be trusted. The next chapter
presents what those runs measured.