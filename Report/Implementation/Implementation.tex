...
This chapter describes how the testbed was built and how the experiments are run on it. It covers the DTN and mail layers, the delay mechanism, the resolver, the setup of each anti-spam tool, and the harness that executes a full set of runs without supervision. This chapter is written in enough detail to help future work that builds on this setup.
...
This contact plan is simple and more-or-less always on: the contact windows are set to 24 hours, since shorter windows expire during a run and bundles stop being forwarded, so the window has to cover a full pass of three runs with margin. A more complex scenario, such as one including an orbiter whose visibility to a ground station comes and goes, would need a contact plan with scheduled outages instead, but that is not needed to study the effect of delay alone.

\section{Mail Layer}

BPMail 0.1.0 provides the gateway between the mail system and ION. It is installed on mail1 and mail2 only; space forwards the bundles between them without running BPMail or speaking any mail protocol, even though those bundles carry mail as their content.
...
BPMail transfers mail over DTPC (Section~\ref{sec:background-ion}) rather than raw bundles, so the two mail nodes need DTPC endpoints and a profile. Service 128 is used for sending and 129 for receiving, and the endpoints are added to each node's bundle protocol configuration as shown above. The DTPC profile is the same on both nodes:
...
The second concerns two timers that interact with the link delay, both given in milliseconds. For the delayed runs both were increased above unbound's defaults, so that they exceed the round-trip time of the delayed link; the reason these values matter, and what happens with the defaults in place, is a result of the experiments and is presented in Chapter~\ref{sec:spamassassin_detection}.

\begin{lstlisting}
discard-timeout: 9000        //default: 1900
infra-cache-min-rtt: 4000    //default: 50
\end{lstlisting}
...
The corpus is drawn from a set of spam messages provided by Richard Clayton, captured from live mail traffic in June 2026. 681 of these were used in every run. Because spam content and the blocklists used to catch it both change over time, this corpus reflects spam as it appeared at that point and is not expected to remain representative, so it is unlikely to be reusable for a repeat of this experiment in future without being refreshed; blocklist listings alone were observed to drift measurably over a two-day gap between runs in this study (Section~\ref{sec:sa-flips}).
...
The harness connects to the three servers with Paramiko, a Python library for scripting SSH sessions. Mail1 and mail2 are not reachable directly from the Mac, so their connections are tunnelled through space, which is opened first.
...
The harness controls the virtual machines through utmctl, UTM's command-line tool. Before anything starts, it checks that utmctl is usable, that the named virtual machines exist, and that the results directory has at least 20 GB free. It then starts the machines in order and waits for each to accept SSH, up to
five minutes. The harness also runs caffeinate, a macOS utility that prevents the machine from sleeping, for the length of the session so the Mac does not go to sleep partway through a run.

\subsection{Checks before a run}
\label{sec:test_harness_verification}

Before a run starts, the harness checks that the testbed is actually in a state where the results will mean something. Any of the checks below, called gates, can stop the run before it begins.

\begin{itemize}
    \item \textbf{Clocks.} The sender on mail1 and the receiver on mail2 need to agree on the time, since their timestamps are compared to measure delay. The harness checks each node's clock against the host and stops the run if either is off by more than a second.
    \item \textbf{Resolver.} unbound has to be running on mail2 and able to answer a test query, or none of the DNS-dependent tools would work.
    \item \textbf{ION.} ION is restarted on all three nodes. This also resets the contact window so it covers the whole run.
    \item \textbf{Tool.} The harness asks the tool for its version, and a response confirms it is installed. That version string is saved alongside the results.
\end{itemize}
...
The sender goes through the corpus in sorted order. For each message it records the Message-ID and a timestamp, sends it, and records the exit code:
