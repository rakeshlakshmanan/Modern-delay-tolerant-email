...
BPMail pipeline for carrying mail over the Bundle Protocol. The same components are
used here, rebuilt on virtual machines rather than containers for the reasons given
in the previous chapter, and are described only in the detail needed to make the
results reproducible.

Everything above that layer is new: the delay mechanism on the relay, the external
resolver, the installation and configuration of the three anti-spam tools, the
measurement method, and the automation that runs the experiments and collects the
results.

\section{Environment and Dependencies}

The testbed runs as five virtual machines on a single Apple Silicon MacBook Air
under UTM. All five run Ubuntu 22.04 LTS (ARM64). Three are servers, mail1, space
and mail2; two are clients, one on each network. The servers carry the mail, DTN and
anti-spam software. The clients run Thunderbird and are used only to confirm that
the mail path works end to end; they take no part in the measurements.

Three libraries are newer than the versions in the Ubuntu 22.04 repositories and are
compiled from source on each server before ION and BPMail are built: mbedtls 2.28.8,
gmime 3.2.7 and c-ares 1.34.4. Each is built and installed in turn, followed by
\texttt{ldconfig} so that the new shared libraries are found ahead of the system
ones.
