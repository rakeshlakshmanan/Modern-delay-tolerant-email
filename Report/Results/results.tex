...
\bottomrule
\end{tabular}
\caption{Aggregate results across the three runs.}
\label{tab:sa-agg}
\end{table}
...
\bottomrule
\end{tabular}
\caption{DNSBL/URIBL rule firing (selected rules); every rule falls from Run~2
to Run~3.}
\label{tab:sa-rules}
\end{table}
...
\bottomrule
\end{tabular}
\caption{DNSBL-hit rate by corpus position; Run~3 collapses after
$\approx$email 500.}
...
\bottomrule
\end{tabular}
\caption{Scan-time distribution (emails per bucket).}
\label{tab:sa-timing}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Verdict changes and blocklist drift, Run~2 to Run~3.}
\label{tab:sa-flips}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Traffic on the relay: DNS to the resolver vs DTN bundle traffic.}
\label{tab:sa-bw}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Detection and scoring across the three runs.}
...
\bottomrule
\end{tabular}
\caption{Scan-time distribution across the three runs (messages per bucket).}
...
\bottomrule
\end{tabular}
\caption{Per-message flips between the two delayed runs, which differ only in
\texttt{task\_timeout}.}
...
\bottomrule
\end{tabular}
\caption{Network volume per run. Capture durations are omitted: the captures
ran unattended, so their wall-clock length does not reflect run time.}
...
\bottomrule
\end{tabular}
\caption{Verdict distribution across the three runs.}
...
\bottomrule
\end{tabular}
\caption{Scan-time distribution across the three runs.}
...
\bottomrule
\end{tabular}
\caption{Per-message flips, matched on a Message-ID and Delivery-date key.
Three Run~1 messages with short duplicate keys are excluded from the left table.}
...
\bottomrule
\end{tabular}
\caption{Network volume per run.}
